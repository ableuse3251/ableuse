...
func _build_ui() -> void:
	...
	teams_row.custom_minimum_size = Vector2(0, 45)
	teams_row.add_theme_constant_override("separation", 20)
	...
	teams_row.add_child(home_name_label)
	...
	teams_row.add_child(away_name_label)
	...
	home_score_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	home_score_label.custom_minimum_size = Vector2(80, 65)
	home_score_label.add_theme_font_size_override("font_size", 52)
	score_row.add_child(home_score_label)

	var colon := Label.new()
	colon.text = ":"
	colon.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	colon.add_theme_font_size_override("font_size", 40)
	colon.add_theme_color_override("font_color", Color(1, 1, 1, 0.4))
	score_row.add_child(colon)

	away_score_label = Label.new()
	away_score_label.text = "0"
	away_score_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	away_score_label.custom_minimum_size = Vector2(80, 65)
	away_score_label.add_theme_font_size_override("font_size", 52)
	score_row.add_child(away_score_label)
	...
	minute_label.add_theme_color_override("font_color", Color(0.3, 0.9, 0.5))
...
func _start_match() -> void:
	...
	match_result = MatchSimulator.simulate_match(team)
...
func _advance_match() -> void:
	...
	progress_bar.value = current_minute
...
func _process_events() -> void:
	...
	while current_event_index < match_result.events.size():
		var event_data: Dictionary = match_result.events[current_event_index]
		var event_minute: int = int(event_data.get("minute", 0))
...
func _finish_match() -> void:
	...
	var final_user_goals: int = int(match_result.get("user_goals", 0))
	var final_opponent_goals: int = int(match_result.get("opponent_goals", 0))

	home_score_label.text = str(final_user_goals)
...
func _show_summary() -> void:
	...
	if summary.has_method("setup_summary"):
		summary.setup_summary(team, match_result)
...
func _on_back_pressed() -> void:
	...
	# ИЗМЕНЕНО: переход на домашний экран
