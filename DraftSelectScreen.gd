...
func _setup_ui() -> void:
	...
	top_glow.size = Vector2(0, 220)
	top_glow.color = Color(0.06, 0.10, 0.19, 0.45)
	...
	center_glow.set_anchors_and_offsets_preset(Control.PRESET_CENTER)
	center_glow.position = Vector2(-260, -170)
	center_glow.size = Vector2(520, 340)
	var glow_style := StyleBoxFlat.new()
	glow_style.bg_color = Color(0.12, 0.16, 0.28, 0.18)
	...
	center_glow.add_theme_stylebox_override("panel", glow_style)
	...
	cards_panel.custom_minimum_size = Vector2(0, CARD_HEIGHT + 24)
	var cards_style := StyleBoxFlat.new()
	cards_style.bg_color = Color(0.035, 0.050, 0.080, 0.78)
	...
	cards_style.border_color = Color(1, 1, 1, 0.08)
	cards_panel.add_theme_stylebox_override("panel", cards_style)
	...
	container.add_theme_constant_override("separation", CARD_GAP)
	...
	spacer_bottom.custom_minimum_size = Vector2(0, 5)
...
func start_choice_for_position(pos_name: Variant = "ПОЗИЦИЮ", choices: Array = []) -> void:
	...
	for card_data in real_choices:
		...
		# ========================================================
			...
			var scene_res = load("res://CardUI.tscn")
			if scene_res:
				card_node = scene_res.instantiate()
			else:
				card_node = Control.new()
				var script_ref = load("res://CardUI.gd")
				if script_ref:
					card_node.set_script(script_ref)
		...
		if card_node:
			...
			card_node.scale = Vector2(0.92, 0.92)
			...
			if card_node.has_signal("card_selected"):
				card_node.card_selected.connect(func(selected_resource):
					player_selected_on_screen.emit(selected_resource)
				)
			...
			# ====================================================
				...
				card_node.pressed.connect(func():
					player_selected_on_screen.emit(card_data)
				)
			...
			tween.parallel().tween_property(card_node, "modulate:a", 1.0, 0.22)
			tween.parallel().tween_property(card_node, "scale", Vector2.ONE, 0.28)
	...
	cards_panel.scale = Vector2(0.98, 0.98)
	var panel_tween := create_tween()
	panel_tween.set_trans(Tween.TRANS_QUAD)
	panel_tween.set_ease(Tween.EASE_OUT)
	panel_tween.tween_property(cards_panel, "modulate:a", 1.0, 0.20)
	panel_tween.parallel().tween_property(cards_panel, "scale", Vector2.ONE, 0.25)
