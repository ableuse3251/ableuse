...
func _build_ui() -> void:
	...
	pack_panel.custom_minimum_size = Vector2(
		0,
		205
	)
	...
	pack_panel.add_theme_stylebox_override(
		"panel",
		pack_style
	)
	...
	pack_box.add_theme_constant_override(
		"separation",
		4
	)
	...
	pack_icon.horizontal_alignment = (
		HORIZONTAL_ALIGNMENT_CENTER
	)

	pack_icon.add_theme_font_size_override(
		"font_size",
		42
	)
	...
	pack_title.horizontal_alignment = (
		HORIZONTAL_ALIGNMENT_CENTER
	)

	pack_title.add_theme_font_size_override(
		"font_size",
		21
	)

	pack_title.add_theme_color_override(
		"font_color",
		Color(
			1.0,
			0.82,
	...
	pack_description.text = (
		"1 случайный игрок из доступной базы"
	)

	pack_description.horizontal_alignment = (
		HORIZONTAL_ALIGNMENT_CENTER
	)

	pack_description.add_theme_font_size_override(
		"font_size",
		13
	)

	pack_description.add_theme_color_override(
		"font_color",
		Color(
			1.0,
			1.0,
	...
	chances.text = (
		"BRONZE 55%  •  SILVER 30%  •  GOLD 12%  •  ELITE 3%"
	)

	chances.horizontal_alignment = (
		HORIZONTAL_ALIGNMENT_CENTER
	)

	chances.autowrap_mode = (
		TextServer.AUTOWRAP_WORD_SMART
	)

	chances.add_theme_font_size_override(
		"font_size",
		11
	)

	chances.add_theme_color_override(
		"font_color",
		Color(
			1.0,
			1.0,
	...
	buy_button.add_theme_font_size_override(
		"font_size",
		17
	)
	...
	result_label.text = (
		"Выберите пак и получите нового игрока."
	)
	...
	result_label.custom_minimum_size = Vector2(
		0,
		65
	)
...
func _on_buy_pressed() -> void:
	...
	# ============================================================
	# ПОЛУЧАЕМ ИГРОКА ДО СПИСАНИЯ МОНЕТ
	# ============================================================
	...
	var random_data: Dictionary = (
		PlayerDatabase.get_random_player_by_rarity(
			rarity
		)
	)
	...
	if random_data.is_empty():
		print(
			"Пак: редкость ",
			rarity,
			" не имеет игроков в базе."
		)

		random_data = (
			PlayerDatabase.get_random_player()
		)

		if random_data.is_empty():
			result_label.text = (
				"❌ В базе игроков нет доступных игроков."
			)
	...
	if not user_profile.spend_coins(
		PACK_PRICE
	):
		result_label.text = (
			"❌ Недостаточно монет!\n"
